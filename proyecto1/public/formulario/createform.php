...
function createform($form,$method,$action,$name)
{
	// For each element
	...
	foreach ($form as $key => $valueform)
	{
		...
		switch ($type)
		{
			...
			case "email":
				...
				$formhtml.=elementinputtext($type,$label,$name,$value,$decorator,$hint,$id);
			...
			case "textarea":
				...
				$formhtml.=elementtextarea($type, $label, $name, $options, $default, $decorator, $hint, $id);
				break;
